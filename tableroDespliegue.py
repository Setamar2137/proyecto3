...
import numpy as np
from conexion_db import get_db_connection  # Importar función para conectar a la base de datos
...
def obtener_datos():
    ...
    if conn:
        try:
            query = "SELECT * FROM datos"  # Consulta para obtener los datos de la tabla
            data = pd.read_sql_query(query, conn)  # Cargar datos en un DataFrame
            conn.close()
            return data
...
# Cargar los modelos desde las rutas en la instancia EC2
model_socioeconomico = load_model('/home/ubuntu/proyecto/modelo_socioeconomico.h5')
model_institucional = load_model('/home/ubuntu/proyecto/modelo_institucional.h5')
